...
- `true`：启用 Cloudflare Turnstile 人机验证，登录、注册发码、密码重置发码、修改密码、修改用户名、注销账户、开启/关闭两步验证等动作均需通过验证，前端展示验证组件
- `false`：上述动作全部跳过人机验证，前端不展示验证组件（登录等页面直接提交）

### 两步验证（TOTP）

基于 RFC 6238 的 TOTP 两步验证（算法为 Go 手写实现，HMAC-SHA1、6 位、30 秒周期、±1 窗口，用 RFC 官方测试向量验证）：

- **启用流程**：Dashboard「安全设置」→ `POST /api/user/totp/setup` 获取密钥与 otpauth:// URI（前端用 @paulmillr/qr 渲染二维码）→ 用户输入验证器 6 位码 → `POST /api/user/totp/enable` 启用并返回 8 个一次性恢复码（SHA-256 哈希存 `totp_recovery_codes` 表，明文仅此一次展示）
- **登录流程**：密码验证通过后签发单次有效的中转 token（内存存储，5 分钟有效，验证失败不消费、成功才消费），前端引导输入 TOTP 码或恢复码完成 `POST /api/auth/login/totp`；恢复码单次使用，原子消费防重放
- **防爆破**：同一用户连续失败 5 次锁定 5 分钟（内存计数，后台任务每分钟清理过期状态），配合 IP 限流（`TOTPLoginRateLimit` 比登录限流更严）
- **关闭流程**：需当前密码 + 有效验证码（TOTP 码或恢复码）双重验证，关闭时清空密钥与恢复码并撤销全部会话（与修改密码策略一致），写入审计日志
- **已知边界**：邮箱验证码重置密码流程暂不要求 TOTP 码（依赖邮箱所有权验证），后续可补充未登录态的两步验证
